...
const num = [1, -2, 3, 5, -6, 3, -10];
const PositiveNumArray = num.filter(function (removeNegative) {
...
const newFruits = fruits.filter(function (a) {
    return a.length > 5;
});
...
const squareEven = even.map(function (a) {
    if (a % 2 == 0) {
        return a * 2;
    }
    else {
...
const farheinheit = celsius.map(function (a) {
    return (a * 9 / 5) + 32;
});
...
const odd = [3, 6, 9, 12, 15, 18];
const doubleOdd = odd.map(function (a) {
    if (a % 2 !== 0) {
        return a * 2;
    }
    else {
